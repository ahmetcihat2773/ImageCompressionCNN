...
class ImageCompression():
    ...
    def jpeg2000(self):
        ...
        current_dir = current_dir + "/" + self.input_folder 
        os.chdir(current_dir)
        for filename in os.listdir(current_dir):
            if "tiff" in filename:                
                cmd = "magick convert {filename} -quality {quality} compressed_{tiff_file}.jp2".format(quality = quality,filename = filename,tiff_file = filename)
                os.system(cmd) 
                number = number + 1       
